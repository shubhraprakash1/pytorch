...
def forward(self, x):
    ...
    def test_cond_raise_user_error_on_missing_args(self):
        ...
        with self.assertRaisesRegex(
            torch._dynamo.exc.UserError,
            re.escape(
                "Fail to execute func with input in eager mode due to exception: cond_dense() missing 1 required positional argument"
            ),
        ):
    ...
    def test_cond_raise_user_error_on_non_list_operands(self):
        ...
        with self.assertRaisesRegex(
            torch._dynamo.exc.UserError,
            re.escape(
                "Fail to execute func with input in eager mode due to exception: <lambda>() takes 1 positional argument but 5 were given"
            ),
        ):
            torch._dynamo.export(
                f_non_list_operands,
